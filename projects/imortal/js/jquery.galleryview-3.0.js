...
*/
	...
	*/
		function buildPanels() {
			...
			if(opts.show_panel_nav) {
				$(".gv-panel-nav-next").css({
					position:'relative',
					//zIndex:'1100',
					top:((opts.filmstrip_position=='top'?opts.frame_gap+wrapper_height:-320)+(opts.panel_height-22)/2)+'px',
					right:((opts.filmstrip_position=='right'?opts.frame_gap+wrapper_width:0)+-17)+'px',
					float: 'right',
					//display:'none'
				}).click(showNextItem);
				$(".gv-panel-nav-prev").css({
					position:'relative',
					//zIndex:'1100',
					top:((opts.filmstrip_position=='top'?opts.frame_gap+wrapper_height:-320)+(opts.panel_height-22)/2)+'px',
					left:((opts.filmstrip_position=='left'?opts.frame_gap+wrapper_width:0)+-17)+'px',
	...
	*/
		function buildFilmstrip() {
			...
			j_filmstrip.css({
				...
				position:'absolute',
				zIndex:'900',
				top:(filmstrip_orientation=='vertical' && opts.filmstrip_style == 'scroll' && slide_method=='strip'?-((f_frame_height+opts.frame_gap)*iterator):17)+'px',
				left:(filmstrip_orientation=='horizontal' && opts.filmstrip_style == 'scroll' && slide_method=='strip'?-((f_frame_width+opts.frame_gap)*iterator):10)+'px',
	...
	*/
		return this.each(function() {
			...
			if(filmstrip_orientation=='horizontal') {
				wrapper_width = ((strip_size*f_frame_width)+((strip_size-0)*opts.frame_gap));
				if(opts.filmstrip_style == 'show all') {
...
*/
	$.fn.galleryView.defaults = {
		...
		panel_height: 327,					//INT - height of gallery panel (in pixels)
		panel_animation: 'crossfade',		//STRING - animation method for panel transitions (crossfade,fade,slide,zoomOut,none)
		overlay_opacity: 0.2,				//FLOAT - transparency for panel overlay (1.0 = opaque, 0.0 = transparent)
		...
		frame_width: 96,					//INT - width of filmstrip frames (in pixels)
